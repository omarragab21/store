...
if($result['image']) {
	...
	$image = $this->model_tool_image->resize('no_image.png', 150, 150);
				...
				$data['categories'][] = array(
					'name' => $result['name'],
					'badge' => $this->config->get('config_product_count') ? ' (' . $this->model_catalog_product->getTotalProducts($filter_data) . ')' : '',
					'image' => $image,
...
$category_link = '';
			...
			if ($category_id==0) { 
				...
				if ($page == 1) {
					...
					if ($page > 1) {
					    $this->document->addLink($this->url->link('product/category', 'path=' . $category_id . (($page - 2) ? '&page='. ($page - 1) : '')), 'prev');
					}
