        ...
        // Xtensions Best Checkout
    		if ($this->xtensions_checkout->isActive('xtensions_best_checkout', $this->config->get('config_store_id'))) {
    			...
    			if(isset($modData['xconfig']['options']['show_country_code']) && $modData['xconfig']['options']['show_country_code']){
    				$data['has_country_code'] = $this->xtensions_checkout->getXtensionsModel($this->config->get('xtensions_custom_fields_model_path'))->getCountryCode($this->customer->getId());	
    			}else{
    				$data['has_country_code'] = true;
    			}
