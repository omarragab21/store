...
class ModelExtensionModuleTestimonial extends Model
{
    ...
    {
        $this->db->query("INSERT INTO " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['name']) . "', job = '" . $this->db->escape($data['job']) . "', customer_id = '" . (int)$this->customer->getId() . "', product_id = '0', text = '" . $this->db->escape($data['text']) . "', rating = '" . (int)$data['rating'] . "', date_added = NOW()");
    ...
    public function getReviews($start = 0, $limit = 20)
    {
        ...
        $query = $this->db->query("SELECT r.review_id, r.customer_id, r.author,r.job, r.rating, r.text, r.date_added FROM " . DB_PREFIX . "review r WHERE r.product_id = '0'  AND r.status = '1'  ORDER BY r.date_added DESC LIMIT " . (int)$start . "," . (int)$limit);
