...
//        if ($currency != $paymentCurrencyIso) {
        ...
        if (isset($baseCurrencyRate) && isset($gatewayCurrencyRate)) {
            ...
            $baseAmount = ceil($totalAmount / $baseCurrencyRate * 100) / 100;
    ...
    public static function getMyFatoorahCountries() {
        ...
        if (file_exists($cachedFile)) {
            if ((time() - filemtime($cachedFile) > 3600)) {
                $cache = file_get_contents('https://portal.myfatoorah.com/Files/API/mf-config.json');
                if ($cache) {
                    file_put_contents($cachedFile, $cache);
