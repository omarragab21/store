					...
					foreach( $data as $type => $vals ) {
						switch( $type ) {
							...
							case 'layouts' : {
								...
								foreach( $vals['setting'] as $row ) {
									if( isset( $row['setting']['store_id'] ) ) {
										$row['setting']['store_id'] = $this->checkImportValues( $row['setting']['store_id'], $to_import['settings']['stores'] );
									}

									if( isset( $row['setting']['layout_id'] ) ) {
										$row['setting']['layout_id'] = $this->checkImportValues( $row['setting']['layout_id'], $to_import['settings']['layouts'] );
									}

									if( isset( $row['setting']['category_id'] ) ) {
										$row['setting']['category_id'] = $this->checkImportValues( $row['setting']['category_id'], $to_import['settings']['categories'] );
									}

									if( isset( $row['setting']['hide_category_id'] ) ) {
										$row['setting']['hide_category_id'] = $this->checkImportValues( $row['setting']['hide_category_id'], $to_import['settings']['categories'] );
									}

									if( isset( $row['setting']['customer_groups'] ) ) {
										$row['setting']['customer_groups'] = $this->checkImportValues( $row['setting']['customer_groups'], $to_import['settings']['customer_groups'] );
									}

									if( isset( $row['setting']['attribs'] ) ) {
										$row['setting']['attribs'] = $this->checkImportAttributes( $row['setting']['attribs'], $to_import['settings'] );
									}

									if( isset( $row['setting']['options'] ) ) {
										$row['setting']['options'] = $this->checkImportOther( $row['setting']['options'], $to_import['settings']['options'] );
									}

									if( isset( $row['setting']['filters'] ) ) {
										$row['setting']['filters'] = $this->checkImportOther( $row['setting']['filters'], $to_import['settings']['filters'] );
									}
									
									if( isset( $row['setting']['categories'] ) ) {
										foreach( $row['setting']['categories'] as $k2 => $v2 ) {
											if( isset( $v2['root_category_id'] ) && $v2['root_category_id'] !== '' ) {
												if( 
													isset( $to_import['settings']['categories'][$row['setting']['categories'][$k2]['root_category_id']] ) &&
													$to_import['settings']['categories'][$row['setting']['categories'][$k2]['root_category_id']] !== ''
												) {
													$row['setting']['categories'][$k2]['root_category_id'] = $to_import['settings']['categories'][$row['setting']['categories'][$k2]['root_category_id']];
												} else {
													$row['setting']['categories'][$k2]['root_category_id'] = null;
												}
											}
										}
									}
									...
									$this->db->query("
										...
										SET
											...
											`code` = 'mega_filter',
											`setting` = '" . $this->db->escape( json_encode( $row['setting'] ) ) . "'
									");
								}
