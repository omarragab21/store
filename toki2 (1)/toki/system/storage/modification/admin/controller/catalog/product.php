...
public function prodstchngsave() {
				if(isset($this->request->post['id']) && $this->request->post['id']) { 
					$status = (isset($this->request->post['status']) && $this->request->post['status']) ? 1 : 0;
					$this->db->query("UPDATE " . DB_PREFIX . "product SET status = '" . $this->db->escape($status) . "' WHERE product_id = '" . (int)$this->request->post['id'] . "'");
				}
			}
	protected function getList() {
if(file_exists('model/extension/prodstchng.php')) {
				$this->load->model('extension/prodstchng');
				$this->model_extension_prodstchng->checkdb();
				$this->document->addScript('view/javascript/prodstchng.js');
				$this->document->addStyle('view/javascript/prodstchng.css');
			}
		if (isset($this->request->get['filter_name'])) {
...
'status' => $result['status'] ? '<label class="oct_switch"> <input type="checkbox" checked class="chkstatus" data-str="'.$result['product_id'].'"/><span class="oct_slider oct_round"></span></label>' : '<label class="oct_switch"> <input type="checkbox" class="chkstatus" data-str="'.$result['product_id'].'"/><span class="oct_slider oct_round"></span></label>',
	...
	protected function getForm() {
		...
		if (isset($this->request->post['express'])) {
			...
			$data['express'] = $product_info['express'];
		} else {
			$data['express'] = 0;
		}
