...
class ControllerTestimonialTestimonial extends Controller
{
    ...
    public function index()
    {
        ...
        if(substr(VERSION, 0, 7) > '2.1.0.2'){
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/' . $this->modulePath . '.twig')) {
                $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/' . $this->modulePath . '.twig', $data));
            } else {
                $this->response->setOutput($this->load->view('default/template/' . $this->modulePath . '.twig', $data));
            }
        }
    }
    ...
    public function review() {
        ...
        if(substr(VERSION, 0, 7) > '2.1.0.2'){
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/' . $this->modulePath . '_list.twig')) {
                $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/' . $this->modulePath . '_list.twig', $data));
            } else {
                $this->response->setOutput($this->load->view('default/template/' . $this->modulePath . '_list.twig', $data));
            }
        }
    }
    ...
    public function write() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if ((utf8_strlen($this->request->post['job']) < 3) || (utf8_strlen($this->request->post['job']) > 255)) {
